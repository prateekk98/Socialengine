...
<div class="global_form_popup">
  ...
  <ul>
    <?php foreach( $this->members as $membership ):
      ...
      ?>
      <li>
        <?php echo $this->htmlLink($member->getHref(), $this->itemPhoto($member, 'thumb.icon'), array('class' => 'profile_friends_icon','target'=>'_blank')) ?>
        <?php echo $this->htmlLink($member->getHref(), $member->getTitle(),array('target'=>'_blank')) ?>
      </li>
    <?php endforeach;?>
  </ul>
  </div>
