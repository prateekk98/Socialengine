...
class Formatter
{
    ...
    public static function prettify($css)
    {
        ...
        $css = preg_replace_callback('#(url\(data:[^\)]+\))#', array('Formatter', '_encode'), $css);
        ...
        $css = preg_replace_callback('#esc\(([^\)]+)\)#', array('Formatter', '_decode'), $css); // unescape escaped blocks
