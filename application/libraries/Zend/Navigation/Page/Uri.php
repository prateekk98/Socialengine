...
class Zend_Navigation_Page_Uri extends Zend_Navigation_Page
{
    ...
    public function isActive($recursive = false)
    {
        if (null === $this->_active) {
            ...
            if(preg_match('/^(http|https):/i',$this->getUri())){
                $requestUri = (_ENGINE_SSL ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].$request->getRequestUri();
                if($requestUri == $this->getUri()){
                    $this->_active = true;
                    return true;
                }
            } else if($request->getRequestUri() == $this->getUri()){
                $this->_active = true;
                return true;
            }
            $this->_active = false;
