 ...
 * @version    $Id: Utilities.php 10196 2014-05-05 20:13:49Z lucas $
...
class Engine_Package_Utilities
{
  ...
  static public function fsRmdirRecursive($path, $includeSelf = false, $skip = array())
  {
    $skip[] = '.';
    $skip[] = '..';
    ...
    foreach( $it as $key => $child ) {
      if( in_array($child->getFilename(), $skip) ) {
        continue;
      }
      if( $it->isDir() ) {
        if( !rmdir($key) ) {
