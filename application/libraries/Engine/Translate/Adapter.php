...
abstract class Engine_Translate_Adapter extends Zend_Translate_Adapter
{
  ...
  public function addTranslation($options = array())
  {
    ...
    if( is_string($options['content']) and is_dir($options['content']) ) {
      ...
      $iterator = new RecursiveIteratorIterator(
        ...
        new RecursiveDirectoryIterator($options['content'], RecursiveDirectoryIterator::KEY_AS_PATHNAME), '/^(?!.*(\.svn|\.cvs)).*$/', RecursiveRegexIterator::MATCH
        ), RecursiveIteratorIterator::SELF_FIRST
      );
      // $customLanguageFiles will contain "custom.csv" files which need to be re-added at the very end
      $customLanguageFiles = array();
      foreach( $iterator as $directory => $info ) {
        ...
        if( $info->isDir() ) {
          ...
          if( $options['scan'] === self::LOCALE_FILENAME ) {
            ...
            if( Zend_Locale::isLocale((string) $filename, true, false) ) {
              ...
              foreach( $parts as $token ) {
                $partsTwo += explode('_', $token);
              }
              $parts = array_merge($parts, $partsTwo);
              $partsTwo = array();
              foreach( $parts as $token ) {
                $partsTwo += explode('-', $token);
              }
              $parts = array_merge($parts, $partsTwo);
          ...
          try {
            ...
            //store custom.csv's into an array to be added last
            if( preg_match('~/custom\.csv$~', $info->getPathname()) ) {
              $customLanguageFiles[(string) $options['locale']] = $options;
              continue;
            }
      ...
      unset($iterator);
      //lastly, load custom.csv's
      foreach( $customLanguageFiles as $locale => $options ) {
        try {
          $this->_addTranslationData($options);
        } catch( Zend_Translate_Exception $e ) {
          // ignore failed sources while scanning
  ...
  public function setOptions(array $options = array())
  {
    ...
    foreach( $options as $key => $option ) {
      if( $key == 'locale' ) {
        $locale = $option;
      } elseif( (isset($this->_options[$key]) and ( $this->_options[$key] != $option)) or ! isset($this->_options[$key]) ) {
  ...
  public function translate($messageId, $locale = null)
  {
     ...
     * So, '\r\n' will result in a four character string containing a slash, an 'r', another slash and an 'n',
     * whereas "\r\n" will contain two characters, those being the new line and carriage return characters
     * So, We need to replace "\r", "\n" with '\r', '\n' before the trnslation.
     */
    if( is_string($messageId) ) {
      $messageId = str_replace(array("\r", "\n"), array('\r', '\n'), $messageId);
    }
    $translatedMessageId = $this->_translate($messageId, $locale, $plural, $number, $plocale);
    if( !is_string($translatedMessageId) ) {
   ...
   *                                       locale identifier, @see Zend_Locale for more information
   * @param  string|array       $plural Translation string, or Array for plural translations
   * @param  string|int         $number
   * @param  string|Zend_Locale $plocale Locale/Language to use
   * @return string
   */
  protected function _translate($messageId, $locale, $plural, $number, $plocale)
  {
