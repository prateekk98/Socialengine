...
class Engine_Service_ReCaptcha2 extends Zend_Service_Abstract
{
  /**
   ...
   *
   * @var Engine_Service_ReCaptcha2_Response
   */
   ...
   * @param array|Zend_Config $params
   * @return Engine_Service_ReCaptcha2
   * @throws Engine_Service_ReCaptcha2_Exception
   */
  public function setParams($params)
  {
    ...
    if( is_array($params) ) {
      ...
      throw new Engine_Service_ReCaptcha2_Exception(
  ...
  public function getHtmlHead()
  {
    ...
    if( !empty($this->_params['loadJs']) ) {
$return = <<<HTML
<script type="text/javascript">
  {$this->_params['loadJs']}("{$host}.js{$langPart}{$renderPart}{$onloadPart}");
</script>
HTML;
    return $return;
    }
...
HTML;
  ...
  public function getHtmlBody()
  {
    if( $this->_publicKey === null ) {
      throw new Engine_Service_ReCaptcha2_Exception('Missing public key');
    }
    ...
    $dataAttributes = 'data-sitekey="' . $this->_publicKey . '"';
    ...
    foreach( $attributes as $key => $attribute ) {
      ...
      $attribute = $key == 'tabindex' ? intval($attribute) : htmlentities($attribute);
      $dataAttributes .= ' data-' . $key . '="' . $attribute . '"';
    }

    $return = <<<HTML
<div class="{$elemClass}" {$dataAttributes}></div>
HTML;
    return $return;
  }
  ...
  protected function _post($responseField)
  {
    if( $this->_privateKey === null ) {
      throw new Engine_Service_ReCaptcha2_Exception('Missing private key');
    }

    if( $this->_ip === null ) {
      throw new Engine_Service_ReCaptcha2_Exception('Missing ip address');
    }
   ...
   * Engine_Service_ReCaptcha2_Response object.
   *
   * @param string $responseField
   * @return Engine_Service_ReCaptcha2_Response
   */
  public function verify($responseField)
  {
    if( empty($responseField) ) {
      throw new Engine_Service_ReCaptcha2_Exception('Missing response field');
    }
    $response = $this->_post($responseField);

    return new Engine_Service_ReCaptcha2_Response(null, null, $response);
  }
}
