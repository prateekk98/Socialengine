 ...
 * @version    PhpfoxAbstract.php 2015-07-30 00:00:00Z john $
 * @author     John
 */
abstract class Install_Import_PhpfoxAbstract
{
...
// Special
  ...
  {
    return array(
      '_fromPath', '_fromPrefix', '_toPath', '_toTable', '_toTableTruncate',
      '_toTableExists', '_messages', '_priority', '_requires', '_verbosity',
      '_hasError', '_hasWarning', '_params', '_batchCount', '_isComplete',
      '_staticSalt', '_currencyType', '_fromOrderBy', '_warningMessage',
      '_settingEnable', '_customFields', '_fromGroupBy', '_fileSize', '_messageCount'
    );
  }
  ...
  public function getFromPrefix()
  {
    if( null === $this->_fromPrefix ) {
      throw new Engine_Exception('No prefix');
    }

    return $this->_fromPrefix;
  }

  public function setFromPrefix($fromPrefix)
  {
  ...
  public function getLog()
  {
    if( null === $this->_log ) {
      $log = new Zend_Log();
      $log->addWriter(new Zend_Log_Writer_Stream(APPLICATION_PATH . '/temporary/log/import-phpfox.log', 'a'));
      $this->_log = $log;
    }
    return $this->_log;
  }
      ...
      if( false != ($email = $this->getParam('email')) && in_array('warning', (array) $this->getParam('emailOptions')) ) {
        try {
          ...
          $mail
            ...
            ->addTo($email)
            ->setSubject('SocialEngine PHPFox Migration Progress for ' . $_SERVER['HTTP_HOST'])
            ->setBodyText("Hello,

This is a SocialEngine PHPFox migration status report.
...
Your Server")
  ...
  protected function _logFile($message, $level = Zend_Log::INFO)
  {
    if( $message instanceof Exception ) {
      $message = $message->getMessage();
    }
