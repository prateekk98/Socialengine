...
class Install_Import_Ning_UserUsers extends Install_Import_Ning_Abstract
{
  ...
  {
    //Delete the users who are not a super user
    $this->getToDb()->delete('engine4_users', array(
      'level_id <> ?' => '1'
    ));
    ...
    foreach ($users as $userId) {
      $this->_insertPrivacy($this->_resourceType, $userId, 'view');
      $this->_insertPrivacy($this->_resourceType, $userId, 'comment');
    }
  }
  ...
  protected function  _translateRow(array $data, $key = null)
  {
    ...
    if( !$this->isUpdateUser($userIdentity) ) {
      try {
        $this->_insertPrivacy($this->_resourceType, $newData['user_id'], 'view');
        $this->_insertPrivacy($this->_resourceType, $newData['user_id'], 'comment');
      } catch( Exception $e ) {

      }
    }
    // search
    $this->_insertSearch($this->_resourceType, $newData['user_id'], array(
      'title' => $newData['displayname'],
    ));
